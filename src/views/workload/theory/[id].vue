...
// 使用全局配置
const { scaleOptions, coefficientOptions, fetchWorkloadConfig } = useWorkloadConfig();
...
<template>
  <div class="h-full">
    ...
    <!-- 配置错误提示 -->
    <NAlert
      v-if="scaleOptions.length === 0 || coefficientOptions.length === 0"
      title="配置加载失败"
      type="error"
      class="mb-16px"
    >
    ...
    <div class="mt-24px">
      <NCard>
        <NForm
          ...
          size="medium"
          :disabled="scaleOptions.length === 0 || coefficientOptions.length === 0"
        >
